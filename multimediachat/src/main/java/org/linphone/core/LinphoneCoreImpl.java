...
class LinphoneCoreImpl implements LinphoneCore {
	...
	private native void setNoiseThreshold(long nativePtr, float ng_thres);
	...
	public void setContext(Object context) {
		...
		if (Version.sdkAboveOrEqual(Version.API12_HONEYCOMB_MR1_31X)) {
			...
			WifiLock lock = wifiManager.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "linphonecore ["+ nativePtr+"] wifi-cii_lock");
			lock.setReferenceCounted(true);
			setAndroidWifiLock(nativePtr,lock);
		}
		if (Version.sdkAboveOrEqual(Version.API14_ICE_CREAM_SANDWICH_40)) {
			WifiManager wifiManager=(WifiManager) mContext.getSystemService(Context.WIFI_SERVICE); 
			MulticastLock lock = wifiManager.createMulticastLock("linphonecore ["+ nativePtr+"] multicast-cii_lock");
			lock.setReferenceCounted(true);
	...
	public synchronized void setNoiseThreshold(float ng_thres) {
			setNoiseThreshold(nativePtr, ng_thres);
	}
	...
	public void enableSpeaker(boolean value) {
		...
		if (call != null && call.getState() == State.StreamsRunning && Hacks.needGalaxySAudioHack()) {
			...
			if (call== null  || (call != null && (call.getState() == State.OutgoingRinging || call.getState() == State.OutgoingInit || call.getState() == State.OutgoingProgress)) || Version.sdkAboveOrEqual(Version.API19_KITKAT_44))
