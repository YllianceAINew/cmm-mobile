...
public class BluetoothManager extends BroadcastReceiver {
			...
			mProfileListener = new BluetoothProfile.ServiceListener() {
				public void onServiceConnected(int profile, BluetoothProfile proxy) {
				    if (profile == BluetoothProfile.HEADSET) {
						...
						Log.d("onServiceConnected() mBlueToothHeadSet : " + mBluetoothHeadset);
				    }
				}
	...
	public boolean routeAudioToBluetooth() {
		...
		if (mBluetoothAdapter != null && mBluetoothAdapter.isEnabled() && mAudioManager != null && mAudioManager.isBluetoothScoAvailableOffCall()) {
			...
			mHandler.postDelayed(new Runnable() {
				...
				public void run() {
					...
					if (isBluetoothHeadsetAvailable()) {
						Log.d("blue routeAudioToBluetooth Success");
						if (mAudioManager != null && !mAudioManager.isBluetoothScoOn()) {
							Log.d("blue mAudioManager != null : mAudioManager.isBluetoothScoOn(): ", mAudioManager.isBluetoothScoOn());
						...
						Log.d("blue routeAudioToBlueTooth Failed");
						//return;
					}
					if (mAudioManager != null && !ok) {
						mAudioManager.setBluetoothScoOn(true);
						mAudioManager.startBluetoothSco();
						ok = isUsingBluetoothAudioRoute();
						Log.d("JHK blue return value retries : " + ok);
					}
				}
			}, 400);
			...
			mIsOkay = isUsingBluetoothAudioRoute();
			mRetries = 0;
			new Thread(new Runnable() {
				@Override
				public void run() {
					while (!mIsOkay && mRetries < 10) {
						try {
							Thread.sleep(200);
						...
						mHandler.postDelayed(new Runnable() {
							...
							public void run() {
								if (mAudioManager != null) {
									...
									mAudioManager.startBluetoothSco();
									mIsOkay = isUsingBluetoothAudioRoute();
									Log.d("JHK blue return value retries : " + mIsOkay + ":" + mRetries);
									mRetries++;
								}
							}
						}, 200);
...
*/
			...
			Log.d("JHK blue return value : " + mIsOkay);
			return mIsOkay;
		}
		Log.d("JHK Bluetooth not For All!!!");
		return false;
	}
	
	public boolean isUsingBluetoothAudioRoute() {
		Log.d("blue mBluetoothDevice" + mBluetoothDevice + ":" + isScoConnected);
	...
	public boolean isBluetoothHeadsetAvailable() {
		...
		if (mBluetoothAdapter != null && mBluetoothAdapter.isEnabled() && mAudioManager != null && mAudioManager.isBluetoothScoAvailableOffCall()) {
			...
			if (mBluetoothHeadset != null) {
				...
				Log.d(" Bluetooth Devices :" + devices);
	...
	public void disableBluetoothSCO() {
		Log.d("blue Disable bluetooth sco");
		Log.d("blue audioManager: ", mAudioManager.isBluetoothScoOn());
		if (mAudioManager != null){// && mAudioManager.isBluetoothScoOn()) {
