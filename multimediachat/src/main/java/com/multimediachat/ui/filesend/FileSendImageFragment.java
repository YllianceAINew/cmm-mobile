...
public class FileSendImageFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        fragment_view = inflater.inflate(R.layout.fragement_image, container, false);
    ...
    private void initUI() {
        ...
        listAdapter = new ImageListAdapter(mContext, mActivity.mImageCursor);
        listView.setAdapter(listAdapter);
    }

    public void updateView() {
        if (listAdapter != null) {
            listAdapter.changeCursor(mActivity.mImageCursor);
    ...
    private class ImageListAdapter extends CursorRecyclerViewAdapter<ImageListAdapter.MyViewHolder> {
        ...
        public ImageListAdapter(Context mContext, Cursor audioCursor) {
        ...
        public void onBindViewHolder(MyViewHolder viewHolder, Cursor cursor) {
            int dataColumn = cursor.getColumnIndex(MediaStore.Files.FileColumns.DATA);
            String path = cursor.getString(dataColumn);
            Glide.with(mContext).setDefaultRequestOptions(new RequestOptions().diskCacheStrategy(DiskCacheStrategy.NONE).format(DecodeFormat.PREFER_RGB_565).placeholder(R.drawable.nophotos_centercrop).error(R.drawable.nophotos_centercrop)).load(path).into(viewHolder.imageView);
            if (mActivity.mCheckedFiles.indexOf(path) >= 0) {
