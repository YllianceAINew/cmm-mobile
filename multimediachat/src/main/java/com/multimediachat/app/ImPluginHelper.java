...
public class ImPluginHelper {
    ...
    public long createAdditionalProvider(String name) {
        ...
        try {
           providerId = insertProviderDb(plugin, pluginInfo, providerFullName, signUpUrl);
        	...
        	DebugConfig.error(TAG, "Storage full", e);
            return -1;
        }
    ...
    public void loadAvailablePlugins() {
        ...
        for (ResolveInfo info : plugins) {
        	DebugConfig.debug(TAG, "Found plugin " + info);

            ServiceInfo serviceInfo = info.serviceInfo;
            if (serviceInfo == null) {
            	DebugConfig.debug(TAG, "Ignore bad IM plugin: " + info);
                continue;
            }
            ...
            if (TextUtils.isEmpty(providerName) || TextUtils.isEmpty(providerFullName)) {
            	DebugConfig.debug(TAG, "Ignore bad IM plugin: " + info + ". Lack of required meta data");
                continue;
            }

            if (isPluginDuplicated(providerName)) {
            	DebugConfig.debug(TAG, "Ignore duplicated IM plugin: " + info);
                continue;
            }

            if (!serviceInfo.packageName.equals(mContext.getPackageName())) {
            	DebugConfig.debug(TAG, "Ignore plugin in package: " + serviceInfo.packageName);
                continue;
            }
            ImPluginInfo pluginInfo = new ImPluginInfo(providerName, serviceInfo.packageName,
                    serviceInfo.name, serviceInfo.applicationInfo.sourceDir);
            ...
            ImPlugin plugin = loadPlugin(pluginInfo);
            if (plugin == null) {
            	DebugConfig.debug(TAG, "Ignore bad IM plugin");
                continue;
            }
    ...
    private ImPlugin loadPlugin(ImPluginInfo pluginInfo) {
        ...
        try {
        	...
        	DebugConfig.error(TAG, "Could not find plugin class", e);
        } catch (IllegalAccessException e) {
        	DebugConfig.error(TAG, "Could not create plugin instance", e);
        } catch (InstantiationException e) {
        	DebugConfig.error(TAG, "Could not create plugin instance", e);
        } catch (SecurityException e) {
        	DebugConfig.error(TAG, "Could not load plugin", e);
    ...
    private long insertProviderDb(ImPlugin plugin, ImPluginInfo info, String providerFullName,
        ...
        Cursor c = cr.query(Imps.Provider.CONTENT_URI, new String[]{Imps.Provider._ID}, where,
                selectionArgs, null);

        if ( c != null && c.moveToFirst()) {
            long id = c.getLong(c.getColumnIndexOrThrow(Imps.Provider._ID));
            c.close();
            return id;
        } else if (c != null) {
            c.close();
