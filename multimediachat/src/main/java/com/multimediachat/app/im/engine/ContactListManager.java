...
public abstract class ContactListManager {
    ...
    public static final String BlOCKED_LIST_NAME = "blocked_list";
    public static final String HIDDEN_LIST_NAME = "hidden_list";
    ...
    protected Vector<Contact>    mHiddenList;
    protected Vector<Contact>    mFavoriteList;
    protected Vector<Contact>    mTemporaryList;
     ...
     * block operation.
     */
    protected Vector<String> mBlockPending;
    /**
    ...
    protected ContactListManager() {
        ...
        mHiddenList = new Vector<Contact>();
        mFavoriteList = new Vector<Contact>();
        mTemporaryList = new Vector<Contact>();
    ...
    protected abstract void fillHiddenBlockedFavoriteList();
    /**
     ...
     * 
     * @param listener the listener to cii_remove.
     */
     ...
     */
    public int blockContactAsync(String address) throws ImException {
        checkState();

        if (isBlocked(address)) {
        	notifyBlockContact(getContact(address), true);
        ...
        return doBlockContactAsync(address, true);
    }
     ...
     */
    public int unblockContactAsync(String address) throws ImException {
        checkState();
        if ( mBlockedList.size() == 0)
        {
        	notifyBlockContact(getContact(address), false);
        	return ImErrorInfo.NO_ERROR;
       		//fillBlockedList();
        ...
        return doBlockContactAsync(address, false);
    }
    
    //cys
    public void hideContactAsync(Contact contact) throws ImException {
        hideContactAsync(contact.getAddress().getAddress());
    }
    
    public int hideContactAsync(String address) throws ImException {
        checkState();

        if (isHidden(address)) {
        	
        	notifyHideContact(getContact(address), true);
        	return ImErrorInfo.NO_ERROR;
        }

        return doHideContactAsync(address, true);
    }
    
    public int unHideContactAsync(Contact contact) throws ImException {
        return unHideContactAsync(contact.getAddress().getAddress());
    }
    
    public int unHideContactAsync(String address) throws ImException {
        checkState();

        if (!isHidden(address)) {
        	notifyHideContact(getContact(address), false);
        	return ImErrorInfo.NO_ERROR;
        }

        return doHideContactAsync(address, false);
    }
    
    public int favoriteContact(String address) throws ImException{
    	checkState();
    	if ( isFavorite(address) ) {
    		notifyFavoriteContact(getContact(address), true);
    		return ImErrorInfo.NO_ERROR;
    	}
    	return doFavoriteContact(address, true);
    }
    
    public int unFavoriteContact(String address) throws ImException{
    	checkState();
    	if ( !isFavorite(address)) {
    		notifyFavoriteContact(getContact(address), false);
    		return ImErrorInfo.NO_ERROR;
    	}
    	return doFavoriteContact(address, false);
    }
     ...
     * @throws ImException 
     */
    public int setContactName(String address, String name) throws ImException {
        checkState();

        int res = ImErrorInfo.NO_ERROR; 
        res= doSetContactName(address,name);
        updateCache(address,name); // used to refresh the display
        return res;
    }
    
    protected abstract int doSetContactName(String address, String name) throws ImException;
    ...
    //cys
    public List<Contact> getHiddenList() throws ImException {
    	checkState();
    	return Collections.unmodifiableList(mHiddenList);
    }
    ...
    //cys
    public boolean isHidden(Contact contact) throws ImException {
        return isHidden(contact.getAddress().getAddress());
    }
    ...
    //cys
    public synchronized boolean isHidden(String address) throws ImException {
        for (Contact c : mHiddenList) {
            if (c.getAddress().getAddress().equals(address)) {
                return true;
            }
        }
    ...
    public synchronized boolean isFavorite(String address) throws ImException {
        for (Contact c : mFavoriteList) {
            if (c.getAddress().getAddress().equals(address)) {
                return true;
            }
        }
    ...
    /**
     * Checks if a contact is temporary
     */
    public synchronized boolean isTemporary(String address) throws ImException{
    	for (Contact c: mTemporaryList) {
    		if ( c.getAddress().getAddress().equals(address)) {
    			return true;
    		}
    	}
    ...
    /**
     * insert temporary contact
     * @param c
     * @return
     */
//    public void insertTemporary(Contact c) {
//    	notifyAddTemporaryContact(c);
//    }
...
//        }
     ...
     */
    public abstract void loadContactListsAsync(Collection<String> addresses);
    
    public abstract void setContactsToListFromDB();

    public abstract void requestSubscription(Contact contact, String greeting);
    ...
    public abstract void cancelRequest(Contact contact);
     ...
     */
    protected abstract int doBlockContactAsync(String address, boolean block);
    protected abstract int doHideContactAsync(String address, boolean hide);
    protected abstract int doFavoriteContact(String address, boolean favorite);
    ...
    /*
     * 친구의 상태메세지가 변경되였을 때 통지한다.
     */
    public void notifyContactStatusUpdated(Contact contact) {
    	for (ContactListListener listener : mContactListListeners) {
            listener.onContactChange(ContactListListener.CONTACT_STATUS_UPDATED, null, contact);
        }
    }
    
    
    /*
     * 친구의 Avatar가 변경되였을 때 통지한다.
     */
    public void notifyContactAvatarUpdated(Contact contact) {
    	for (ContactListListener listener : mContactListListeners) {
            listener.onContactChange(ContactListListener.CONTACT_AVATAR_UPDATED, null, contact);
        }
    }
    ...
    protected void notifyBlockContact(Contact contact, boolean blocked) {
        synchronized (this) {
            if (blocked) {
            	if ( !mBlockedList.contains(contact))
    ...
    //cys
    protected void notifyHideContact(Contact contact, boolean hidden) {
        synchronized (this) {
            if (hidden) {
            	if ( !mHiddenList.contains(contact))
            		mHiddenList.add(contact);
            } else {
            	mHiddenList.remove(contact);
            }
        }
        for (ContactListListener listener : mContactListListeners) {
            listener.onContactChange(hidden ? ContactListListener.CONTACT_HIDDEN
                                            : ContactListListener.CONTACT_UNHIDDEN, null, contact);
        }
    }
    
    protected void notifyFavoriteContact(Contact contact, boolean favorite) {
    	synchronized (this) {
			if ( favorite ) {
				if ( !mFavoriteList.contains(contact) )
					mFavoriteList.add(contact);
			} else {
				mFavoriteList.remove(contact);
			}
		}
    	
    	for (ContactListListener listener : mContactListListeners) {
            listener.onContactChange(favorite ? ContactListListener.CONTACT_FAVORITE
                                            : ContactListListener.CONTACT_UNFAVORITE, null, contact);
        }
    }
    
    //cys
    protected void notifyAddTemporaryContact(Contact contact) {
        synchronized (this) {
        	if ( !mTemporaryList.contains(contact))
        		mTemporaryList.add(contact);
        }
        
        for (ContactListListener listener : mContactListListeners) {
            listener.onContactChange(ContactListListener.CONTACT_ADDED_TEMPORARY , null, contact);
        }
    }
    ...
    public abstract RosterEntry getRosterEntry(String address);
}
