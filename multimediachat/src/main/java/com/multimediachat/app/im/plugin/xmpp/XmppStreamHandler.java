...
import com.multimediachat.app.im.plugin.xmpp.XmppConnection.MyXMPPConnection;
...
public class XmppStreamHandler {
    ...
    private void sendEnablePacket() {
    	DebugConfig.debug(XmppConnection.TAG,"sm send enable " + sessionId);
        if (sessionId != null) {
    ...
    private void startListening() {
        ...
        mConnection.addPacketSendingListener(new PacketListener() {
            public void processPacket(Packet packet) {
                ...
                if (!isStanza(packet)) {
                    DebugConfig.debug(XmppConnection.TAG,"send " + packet.toXML());
                    return;
                }
                ...
                if (isOutgoingSmEnabled && !outgoingQueue.contains(packet)) {
                    ...
                    DebugConfig.debug(XmppConnection.TAG,"send " + outgoingStanzaCount + " : " + packet.toXML());
                    ...
                    outgoingStanzaCount++;
                    DebugConfig.debug(XmppConnection.TAG,"send DUPLICATE " + outgoingStanzaCount + " : " + packet.toXML());
                } else {
                    DebugConfig.debug(XmppConnection.TAG,"send " + packet.toXML());
                }
            }
        ...
        mConnection.addPacketListener(new PacketListener() {
            public void processPacket(Packet packet) {
                if (isSmEnabled && isStanza(packet)) {
                    incomingStanzaCount++;
                    DebugConfig.debug(XmppConnection.TAG,"recv " + incomingStanzaCount + " : " + packet.toXML());
                } else {
                    DebugConfig.debug(XmppConnection.TAG,"recv " + packet.toXML());
                }
                ...
                if (packet instanceof StreamHandlingPacket) {
                    ...
                    if ("sm".equals(name)) {
                    	DebugConfig.debug(XmppConnection.TAG,"sm avail");
                        isSmAvailable = true;
                        if (sessionId != null)
                            sendEnablePacket();
                        ...
                        DebugConfig.debug(XmppConnection.TAG,outgoingQueue.size() + " in outgoing queue after ack");
                    } else if ("enabled".equals(name)) {
                        incomingStanzaCount = 0;
                        isSmEnabled = true;
                        mConnection.getRoster().setOfflineOnError(false);	//ccj
                        String resume = shPacket.getAttribute("resume");
                        if ("true".equals(resume) || "1".equals(resume)) {
                            sessionId = shPacket.getAttribute("id");
                        }
                        DebugConfig.debug(XmppConnection.TAG,"sm enabled " + sessionId);
                    } else if ("resumed".equals(name)) {
                    	DebugConfig.debug(XmppConnection.TAG,"sm resumed");
                        ...
                        DebugConfig.debug(XmppConnection.TAG,outgoingQueue.size() + " in outgoing queue after resume");
                    	...
                    	DebugConfig.debug(XmppConnection.TAG,"sm failed");
                        mConnection.getRoster().setOfflineOnError(true);	//ccj
                        mConnection.getRoster().setOfflinePresences();	//ccj
                        sessionId = null;
                        mConnection.shutdown();
    ...
    private void removeOutgoingAcked(long ackCount) {
        if (ackCount > outgoingStanzaCount) {
        	DebugConfig.debug(XmppConnection.TAG, "got ack of " + ackCount + " but only sent " + outgoingStanzaCount);
    ...
    public void queue(Packet packet) {
        if (outgoingQueue.size() >= maxOutgoingQueueSize) {
        	DebugConfig.error(XmppConnection.TAG, "outgoing queue full");
            return;
        }
        outgoingStanzaCount++;
        outgoingQueue.add(packet);
