...
import com.multimediachat.util.Crypto;
...
public class ImpsProvider extends ContentProvider {
	...
	private static final String AUTHORITY = "com.multimediachat.app.im.provider.Imps";
	...
	private static final String TABLE_AVATARS_BIG = "big_avatars";
	...
	protected static final int MATCH_MESSAGES_BY_ID = 107;
	...
	protected static final int MATCH_AVATARS_BIG = 73;
	protected static final int MATCH_AVATAR_BIG = 74;
	protected static final int MATCH_AVATAR_BIG_BY_PROVIDER = 75;
	...
	protected static final int MATCH_TAGS = 106;

	protected static final int MATCH_UNICONTACTS = 300;
	protected static final int MATCH_VIEW_UNICONTACTS = 301;
	protected static final int MATCH_PROFILE = 310;
	// protected static final int MATCH_GROUPPROFILE = 311;
	protected static final int MATCH_NOTIFICATIONS = 320;
	protected static final int MATCH_STICKERS = 330;
	protected static final int MATCH_THEMES = 340;
	...
	private static final String DOMAIN_JOIN_ACCOUNT_TABLE = "providerSettings JOIN accounts ON "
			+ "(providerSettings.provider = accounts.provider AND providerSettings.name = '"
			+ Imps.ProviderSettings.DOMAIN + "' AND accounts.active = 1) "
			+ "LEFT OUTER JOIN accountStatus ON (accounts._id = accountStatus.account)";
	...
	private static final String NON_BLOCKED_CONTACTS_WHERE_CLAUSE = "(" + Imps.Contacts.TYPE + " IS NULL OR "
			+ Imps.Contacts.TYPE + "!=" + String.valueOf(Imps.Contacts.TYPE_BLOCKED) + ")";
	...
	private final String[] BACKFILL_PROJECTION = { Imps.Chats._ID, Imps.Chats.SHORTCUT, Imps.Chats.LAST_MESSAGE_DATE };
	...
	private static final String CONTACTS_WITH_NO_PRESENCE_SELECTION = Imps.Contacts.ACCOUNT + "=?" + " AND "
			+ Imps.Contacts._ID + " in (select " + CONTACT_ID + " from " + TABLE_CONTACTS + " left outer join "
			+ TABLE_PRESENCE + " on " + CONTACT_ID + '=' + PRESENCE_CONTACT_ID + " where " + PRESENCE_CONTACT_ID
			+ " IS NULL)";
	...
	private static final String CONTACT_ID_QUERY_SELECTION = Imps.Contacts.ACCOUNT + "=? AND " + Imps.Contacts.USERNAME
			+ "=?";
	...
	private class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			db.execSQL(
					"CREATE TABLE IF NOT EXISTS " + TABLE_PROVIDERS + " (" + "_id INTEGER PRIMARY KEY," + "name TEXT," + // eg
																															// AIM
							"fullname TEXT," + // eg AOL Instance Messenger
							"category TEXT," + // a category used for forming
												// intent
							"signup_url TEXT" + // web url to visit to create a
												// new account
							");");

			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_ACCOUNTS + " (" + "_id INTEGER PRIMARY KEY," + "name TEXT,"
					+ "provider INTEGER," + "username TEXT UNIQUE," + "pw TEXT,"
					+ "active INTEGER NOT NULL DEFAULT 0,"
					+ "locked INTEGER NOT NULL DEFAULT 0," + "keep_signed_in INTEGER NOT NULL DEFAULT 0,"
					+ "last_login_state INTEGER NOT NULL DEFAULT 0," + "UNIQUE (provider, username)" + ");");

			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_CONTACTS + " (_id INTEGER PRIMARY KEY,"
					+ "username TEXT UNIQUE on conflict replace,nickname TEXT,provider INTEGER,account INTEGER,"
					+ "contactList INTEGER,type INTEGER,subscriptionStatus INTEGER,"
					+ "subscriptionType INTEGER,qc INTEGER," + "rejected INTEGER,otr INTEGER,"
					+ "status TEXT, gender TEXT, region TEXT, subscriptionMessage TEXT," // 새로
																											// 추가
					+ "is_delivered INTEGER," // 새로 추가
					+ "deliverType INTEGER," // 새로 추가
					+ "is_deleted INTEGER," // 새로 추가
					+ "favor INTEGER," // 새로 추가
					+ "phone TEXT," // added
					+ "cover_image_count INTEGER," // added
					+ "follow_count INTEGER," // added
					+ "home_url TEXT," // added
					+ "website TEXT," // added
					+ "hash TEXT," // added
					+ "fullname TEXT," // added
					...
					+ "star INTEGER," // added
					+ "userid TEXT," // added
					+ "isloadedvcard INTEGER,"
					+ "thumbnail_path TEXT)"); // 새로 추가

			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_TAGS
					+ " (_id INTEGER PRIMARY KEY,tag TEXT,username TEXT,account INTEGER)");

			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_CONTACTS_ETAG
					+ " (_id INTEGER PRIMARY KEY,etag TEXT,otr_etag TEXT,account INTEGER UNIQUE)");
			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_CONTACT_LIST
					+ " (_id INTEGER PRIMARY KEY,name TEXT,provider INTEGER,account INTEGER)");
			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_BLOCKED_LIST
					+ " (_id INTEGER PRIMARY KEY,username TEXT,nickname TEXT,provider INTEGER,account INTEGER)");

			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_MESSAGES + " (_id INTEGER PRIMARY KEY,"
					+ "thread_id INTEGER,nickname TEXT,body TEXT," + "date INTEGER,server_time INTEGER,type INTEGER,packet_id TEXT UNIQUE,"
					+ "err_code INTEGER NOT NULL DEFAULT 0," + " file_sub_count INTEGER NOT NULL DEFAULT 0," + " file_total_count INTEGER NOT NULL DEFAULT 0,"
					+ "err_msg TEXT,is_muc INTEGER," + "show_ts INTEGER,is_delivered INTEGER," + "time_delay INTEGER," + "thumbnail_width INTEGER,"
					+ "thumbnail_height INTEGER," + "mime_type TEXT," + "plusimage_url TEXT," //
					+ "plusimage_num INTEGER," + "pluslink_url TEXT," + "sample_image_path TEXT)" // 새로
																									// 추가(20141002));
			);

			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_CHATS + " (_id INTEGER PRIMARY KEY,"
					+ "contact_id INTEGER UNIQUE,jid_resource TEXT," + "groupchat INTEGER,last_unread_message TEXT,"
					+ "last_message_date INTEGER," + "notification INTEGER NOT NULL DEFAULT 0," + "chat_room_background TEXT,"
					+ "unsent_composed_message TEXT,shortcut INTEGER)");

			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_IN_MEMORY_MESSAGES
					+ " (_id INTEGER PRIMARY KEY,thread_id INTEGER,"
					+ "nickname TEXT,body TEXT,date INTEGER,server_time INTEGER,type INTEGER,packet_id TEXT UNIQUE,"
					+ "err_code INTEGER NOT NULL DEFAULT 0," + "file_sub_count INTEGER NOT NULL DEFAULT 0," + "file_total_count INTEGER NOT NULL DEFAULT 0,"
					+ "err_msg TEXT,is_muc INTEGER," + "show_ts INTEGER,is_delivered INTEGER,time_delay INTEGER, thumbnail_width INTEGER, thumbnail_height INTEGER, mime_type TEXT,"
					+ "plusimage_url TEXT," + "plusimage_num INTEGER," + "pluslink_url TEXT,"
					+ "sample_image_path TEXT)");

			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_AVATARS + " (" + "_id INTEGER PRIMARY KEY,"
					+ "contact TEXT," + "provider_id INTEGER," + "account_id INTEGER," + "hash TEXT," + "data BLOB," + // raw
																														// image
																														// data
					"UNIQUE (account_id, contact)" + ");");

			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_PROVIDER_SETTINGS + " (" + "_id INTEGER PRIMARY KEY,"
					+ "provider INTEGER," + "name TEXT," + "value TEXT," + "UNIQUE (provider, name)" + ");");

			db.execSQL("create TABLE IF NOT EXISTS " + TABLE_BRANDING_RESOURCE_MAP_CACHE + " ("
					+ "_id INTEGER PRIMARY KEY," + "provider_id INTEGER," + "app_res_id INTEGER,"
					+ "plugin_res_id INTEGER" + ");");

			// clean up account specific data when an account is deleted.
			db.execSQL("CREATE TRIGGER IF NOT EXISTS account_cleanup " + "DELETE ON " + TABLE_ACCOUNTS + " BEGIN "
					+ "DELETE FROM " + TABLE_AVATARS + " WHERE account_id= OLD._id;" + "END");

			// add a database trigger to clean up associated provider settings
			// while deleting a provider
			db.execSQL("CREATE TRIGGER IF NOT EXISTS provider_cleanup " + "DELETE ON " + TABLE_PROVIDERS + " BEGIN "
					+ "DELETE FROM " + TABLE_PROVIDER_SETTINGS + " WHERE provider= OLD._id;" + "END");

			/* version 2 */
			db.execSQL("create table IF NOT EXISTS CONTACT (_id integer primary key autoincrement, " + "no integer, "
					+ "nickname text," + "type integer, " + "picaaddress text," + "facebookaddress text ,"
					+ "phonename text," + "phonenumber text  unique on conflict ignore," + "picaproviderid integer,"
					+ "facebookproviderid integer," + "facebookid text ,"
					+ "picacontactid integer  unique on conflict ignore,"
					+ "facebookcontactid integer  unique on conflict ignore," + "ispica integer,"
					+ "isfacebook integer," + "state integer," + "status text ," + "isphone integer ); ");
			/* version 3 */
			db.execSQL("CREATE TABLE IF NOT EXISTS big_avatars (_id INTEGER PRIMARY KEY,contact TEXT,"
					+ "provider_id INTEGER,account_id INTEGER,hash TEXT,data BLOB," + "UNIQUE (account_id, contact));");

			// presence
			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_PRESENCE + " (" + "_id INTEGER PRIMARY KEY,"
					+ "contact_id INTEGER UNIQUE," + "jid_resource TEXT," + // jid
																			// resource
																			// for
																			// the
																			// presence
			...
			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_INVITATIONS + " (" + "_id INTEGER PRIMARY KEY,"
					+ "providerId INTEGER," + "accountId INTEGER," + "inviteId TEXT," + "sender TEXT,"
					+ "groupName TEXT," + "note TEXT," + "status INTEGER" + ");");

			// group chat members
			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_GROUP_MEMBERS + " (" + "_id INTEGER PRIMARY KEY,"
					+ "groupId INTEGER," + "username TEXT," + "nickname TEXT," + "type INTEGER" + ");");

			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_ACCOUNT_STATUS + " (" + "_id INTEGER PRIMARY KEY,"
					+ "account INTEGER UNIQUE," + "presenceStatus INTEGER," + "connStatus INTEGER" + ");");

			db.execSQL("create view if not exists " + VIEW_UNICONTACTS + " as select uni._id _id,  uni.no no, "
					+ "case when pica.nickname is not null then pica.nickname when facebook.nickname is not null then facebook.nickname else uni.nickname end nickname ,"
					+ "uni.type type,pica.username picaaddress,facebook.username facebookaddress,uni.phonenumber phonenumber, "
					+ "case when pica.provider is null then 0 else pica.provider end picaproviderid , "
					+ "case when facebook.provider is null then 0 else facebook.provider end facebookproviderid, "
					+ "uni.facebookid facebookid, uni.picacontactid picacontactid, "
					+ "uni.facebookcontactid facebookcontactid, case when uni.picacontactid > 0 then 1 else 0 end ispica, "
					+ "case when uni.facebookcontactid > 0 then 1 else 0 end isfacebook ,uni.state state,uni.status status, "
					+ "case when uni.phonenumber is null or uni.phonenumber =''  then 0 else 1 end isphone,"
					+ "case when picapresence.mode is null then 0 else picapresence.mode end  picapresence, "
					+ "case when facebookpresence.mode is null then 0 else facebookpresence.mode end facebookpresence,uni.phonename phonename "
					+ "from CONTACT uni left outer join contacts pica on uni.picacontactid=pica._id "
					+ "left outer join contacts facebook on uni.facebookcontactid=facebook._id " + "left outer join "
					+ TABLE_PRESENCE + " picapresence on uni.picacontactid = picapresence.contact_id "
					+ "left outer join " + TABLE_PRESENCE
					+ " facebookpresence on uni.facebookcontactid = facebookpresence.contact_id");

			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_SESSION_COOKIES + " (" + "_id INTEGER PRIMARY KEY,"
					+ "provider INTEGER," + "account INTEGER," + "name TEXT," + "value TEXT" + ");");

			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_PROFILE
					+ " (nickname text,status text,gender text,publicid text,"
					+ "phone text,region text, hash text, facebookid text, userid text, email text, email_password text, email_logined text, allow_add_me text, "
                    + "delete_old_messages INTEGER ,"
                    + "chat_limit INTEGER,"
					+ "new_message_alert INTEGER NOT NULL DEFAULT 1,"
					+ "video_call_notification INTEGER NOT NULL DEFAULT 1,"
					+ "call_ringtone INTEGER NOT NULL DEFAULT 1,"
					+ "notification_center INTEGER,"
					+ "sound INTEGER NOT NULL DEFAULT 1,"
					+ "alert_sound INTEGER NOT NULL DEFAULT 0,"
					+ "vibrate INTEGER NOT NULL DEFAULT 1,"
					+ "do_not_disturb INTEGER,"
					+ "start INTEGER,"
					+ "end INTEGER,"
					+ "turn_off_speaker INTEGER,"
					+ "press_enter_to_send INTEGER,"
					+ "background TEXT,"
					+ "friend_confirmation INTEGER,"
					+ "find_mobile_contacts INTEGER,"
					+ "methods_multimediachatid INTEGER,"
					+ "methods_phonenumber INTEGER,"
					+ "methods_groupchat INTEGER,"
					+ "methods_qrcode INTEGER,"
					+ "methods_contactcard INTEGER,"
					+ "do_not_share_my_posts TEXT,"
					+ "hide_users_posts TEXT,"
					+ "make_my_posts_public INTEGER,"
					+ "viewable_by_others INTEGER,"
					+ "enable_posts INTEGER,"
					+ "posts_update_alerts INTEGER,"
					+ "landscape_display INTEGER,"
					+ "enable_nfc INTEGER,"
					+ "auto_update INTEGER,"
					+ "text_size INTEGER,"
					+ "features_peoplenearby INTEGER,"
					+ "features_shake INTEGER,"
					+ "is_delivered integer DEFAULT 0)");

			/*
			 * db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_GROUPPROFILE +
			 * " (id text, groupName text, users integer, gmark integer, hash text)"
			 * );
			 */
			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_NOTIFICATIONS
					+ " (_id integer NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE," + "category text,field text,"
					+ "field_id integer,nCount integer,account integer," + "UNIQUE (field, field_id, account))");

			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_STICKERS + " (" + "_id INTEGER PRIMARY KEY," + "name TEXT,"
					+ "category TEXT," + "state INTEGER," + "show_order INTEGER," + "scount INTEGER," // added
					+ "UNIQUE (category)" + ");");

			db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_THEMES + " (" + "_id INTEGER PRIMARY KEY," + "name TEXT,"
					+ "category TEXT," + "UNIQUE (category)" + ");");

			db.execSQL("CREATE INDEX IF NOT EXISTS ind_contacts_username on contacts (username);");
			db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS ind_avatars_username on avatars (contact);");
			db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS ind_bigavatars_username on big_avatars (contact);");
			db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS ind_presence_contactid on presence (contact_id);");
			db.execSQL("CREATE INDEX IF NOT EXISTS ind_message_threadid on messages (thread_id);");
			db.execSQL("CREATE INDEX IF NOT EXISTS ind_contact_picaid on CONTACT (picacontactid);");
			db.execSQL("CREATE INDEX IF NOT EXISTS ind_contact_facebookid on CONTACT (facebookcontactid);");

			db.execSQL("drop trigger if exists contact_cleanup; ");
			db.execSQL("CREATE TRIGGER contact_cleanup " + "DELETE ON " + TABLE_CONTACTS + " BEGIN " + "DELETE FROM "
					+ TABLE_AVATARS + " WHERE contact = OLD.username; " + "DELETE FROM " + TABLE_AVATARS_BIG
					+ " WHERE contact = OLD.username; " + "DELETE FROM " + TABLE_PRESENCE
					+ " WHERE contact_id = OLD._id; " + "DELETE FROM " + TABLE_BLOCKED_LIST
					+ " WHERE username = OLD.username; " + "DELETE FROM " + TABLE_CHATS
					+ " WHERE contact_id = OLD._id; " + "DELETE FROM " + TABLE_MESSAGES + " WHERE thread_id = OLD._id; "
					+ "END");
...
//					db.beginTransaction();
//					try {
//						db.execSQL("drop table if exists contact_backup;");
//						db.execSQL(
//								"CREATE TEMPORARY TABLE IF NOT EXISTS contact_backup(_id integer primary key autoincrement, "
//										+ "no integer, " + "nickname text," + "type integer, " + "phonename text,"
//										+ "picaaddress text," + "facebookaddress text ,"
//										+ "phonenumber text unique on conflict ignore," + "picaproviderid integer,"
//										+ "facebookproviderid integer," + "facebookid text ,"
//										+ "picacontactid integer  unique on conflict ignore, "
//										+ "facebookcontactid integer  unique on conflict ignore,  " + "ispica integer,"
//										+ "isfacebook integer," + "state integer," + "status text ,"
//										+ "isphone integer ); ");
//						db.execSQL(
//								"insert into contact_backup(_id,nickname,type,phonename,phonenumber,facebookid,picacontactid,facebookcontactid,state,status) "
//										+ "select _id,nickname,type,case when length(phonenumber) < 8 then null else nickname end phonename,case when length(phonenumber) < 8 then null else phonenumber end phonenumber,facebookid,"
//										+ "case when picacontactid > 0 then picacontactid else null end picacontactid , "
//										+ "case when facebookcontactid > 0 then facebookcontactid else null end facebookcontactid,"
//										+ "state,status from " + TABLE_UNICONTACTS + ";");
//						db.execSQL("drop table if exists " + TABLE_UNICONTACTS + ";");
//						db.execSQL("create table IF NOT EXISTS  CONTACT (_id integer primary key autoincrement, "
//								+ "no integer, " + "nickname text," + "type integer, " + "picaaddress text,"
//								+ "facebookaddress text ," + "phonename text,"
//								+ "phonenumber text  unique on conflict ignore," + "picaproviderid integer,"
//								+ "facebookproviderid integer," + "facebookid text ,"
//								+ "picacontactid integer  unique on conflict ignore,"
//								+ "facebookcontactid integer  unique on conflict ignore," + "ispica integer,"
//								+ "isfacebook integer," + "state integer," + "status text ," + "isphone integer ); ");
//						db.execSQL("insert into " + TABLE_UNICONTACTS
//								+ "(_id,nickname,type,phonename,phonenumber,facebookid,picacontactid,facebookcontactid,state,status) "
//								+ "select _id,nickname,type,phonename,phonenumber,facebookid,picacontactid,facebookcontactid,state,status from contact_backup;");
//						db.execSQL("CREATE TABLE IF NOT EXISTS big_avatars (_id INTEGER PRIMARY KEY,contact TEXT,"
//								+ "provider_id INTEGER,account_id INTEGER,hash TEXT,data BLOB,"
//								+ "UNIQUE (account_id, contact));");
//						db.execSQL("drop table if exists contact_backup;");
//						db.execSQL("drop view if exists " + VIEW_UNICONTACTS + ";");
...
//						getAccountCount(db);
//
//						db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_AVATARS_BIG + " (" + "_id INTEGER PRIMARY KEY,"
//								+ "contact TEXT," + "provider_id INTEGER," + "account_id INTEGER," + "hash TEXT,"
...
//																						// presence
//								"client_type INTEGER," + // client type
//								"priority INTEGER," + // presence priority
//														// (XMPP)
//								"mode INTEGER," + // presence mode
//								"status TEXT" + // custom status
//								");");
...
//								+ "_id INTEGER PRIMARY KEY," + "groupId INTEGER," + "username TEXT," + "nickname TEXT,"
//								+ "type INTEGER" + ");");
//
//						db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_ACCOUNT_STATUS + " ("
//								+ "_id INTEGER PRIMARY KEY," + "account INTEGER UNIQUE," + "presenceStatus INTEGER,"
//								+ "connStatus INTEGER" + ");");
//
//						db.execSQL(
//								"create view if not exists " + VIEW_UNICONTACTS + " as select uni._id _id,  uni.no no, "
//										+ "case when pica.nickname is not null then pica.nickname when facebook.nickname is not null then facebook.nickname else uni.nickname end nickname ,"
//										+ "uni.type type,pica.username picaaddress,facebook.username facebookaddress,uni.phonenumber phonenumber, "
//										+ "case when pica.provider is null then 0 else pica.provider end picaproviderid , "
//										+ "case when facebook.provider is null then 0 else facebook.provider end facebookproviderid, "
...
//										+ "case when facebookpresence.mode is null then 0 else facebookpresence.mode end facebookpresence,uni.phonename phonename "
//										+ "from CONTACT uni left outer join contacts pica on uni.picacontactid=pica._id "
//										+ "left outer join contacts facebook on uni.facebookcontactid=facebook._id "
//										+ "left outer join " + TABLE_PRESENCE
//										+ " picapresence on uni.picacontactid = picapresence.contact_id "
//										+ "left outer join " + TABLE_PRESENCE
//										+ " facebookpresence on uni.facebookcontactid = facebookpresence.contact_id");
//
//						db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_SESSION_COOKIES + " ("
//								+ "_id INTEGER PRIMARY KEY," + "provider INTEGER," + "account INTEGER," + "name TEXT,"
//								+ "value TEXT" + ");");
//
//						db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_PROFILE
//								+ " (nickname text,status text,gender text,publicid text,"
//								+ "phone text,region text,avatar blob,big_avatar blob,"
//								+ "is_delivered integer DEFAULT 0)");
//
//						/*
//						 * db.execSQL("CREATE TABLE IF NOT EXISTS " +
//						 * TABLE_GROUPPROFILE +
//						 * " (id text, groupName text, users integer, gmark integer, avatar blob)"
//						 * );
//						 */
//						db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_NOTIFICATIONS
//								+ " (_id integer NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE,"
//								+ "category text,field text," + "field_id integer,nCount integer,"
//								+ "UNIQUE (field, field_id))");
//						db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN status text;");
//						db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN facebookid text;");
//						db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN gender text;");
//						db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN region text;");
//						db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN is_delivered INTEGER;");
//						db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN deliverType INTEGER;");
//						db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN is_deleted INTEGER;");
//						/* create index */
//						db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS ind_contacts_username on contacts (username);");
//						db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS ind_avatars_username on avatars (contact);");
//						db.execSQL(
//								"CREATE UNIQUE INDEX IF NOT EXISTS ind_bigavatars_username on big_avatars (contact);");
//						db.execSQL(
//								"CREATE UNIQUE INDEX IF NOT EXISTS ind_presence_contactid on presence (contact_id);");
//						db.execSQL("CREATE INDEX IF NOT EXISTS ind_message_threadid on messages (thread_id);");
//						db.execSQL("CREATE INDEX IF NOT EXISTS ind_contact_picaid on CONTACT (picacontactid);");
//						db.execSQL("CREATE INDEX IF NOT EXISTS ind_contact_facebookid on CONTACT (facebookcontactid);");
//
//						/* create trigger contact cleanup */
//						db.execSQL("drop trigger if exists contact_cleanup; ");
//						db.execSQL("CREATE TRIGGER contact_cleanup " + "DELETE ON " + TABLE_CONTACTS + " BEGIN "
//								+ "DELETE FROM " + TABLE_AVATARS + " WHERE contact = OLD.username; " + "DELETE FROM "
//								+ TABLE_AVATARS_BIG + " WHERE contact = OLD.username; " + "DELETE FROM "
//								+ TABLE_PRESENCE + " WHERE contact_id = OLD._id; " + "DELETE FROM " + TABLE_BLOCKED_LIST
//								+ " WHERE username = OLD.username; " + "DELETE FROM " + TABLE_CHATS
//								+ " WHERE contact_id = OLD._id; " + "DELETE FROM " + TABLE_MESSAGES
//								+ " WHERE thread_id = OLD._id; " + "END");
//
//						db.setTransactionSuccessful();
//					} catch (Throwable ex) {
//						LogCleaner.error(LOG_TAG, ex.getMessage(), ex);
...
//					db.beginTransaction();
//					try {
//						db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN favor integer");
//						db.setTransactionSuccessful();
//					} catch (Throwable ex) {
//						LogCleaner.error(LOG_TAG, ex.getMessage(), ex);
//						break;
...
//					db.beginTransaction();
//					try {
//						db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN isloadedvcard integer");
//						db.execSQL("ALTER TABLE " + TABLE_MESSAGES + " ADD COLUMN time_delay integer");
//						db.execSQL("ALTER TABLE " + TABLE_IN_MEMORY_MESSAGES + " ADD COLUMN time_delay integer");
//						db.setTransactionSuccessful();
//					} catch (Throwable ex) {
//						LogCleaner.error(LOG_TAG, ex.getMessage(), ex);
//						break;
...
//					db.beginTransaction();
//					try {
//						db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN subscriptionMessage TEXT");
//						db.setTransactionSuccessful();
//					} catch (Throwable ex) {
//						break;
//					} finally {
...
//					db.beginTransaction();
//					try {
//						db.execSQL("ALTER TABLE " + TABLE_MESSAGES + " ADD COLUMN thumbnail_width integer");
//						db.execSQL("ALTER TABLE " + TABLE_MESSAGES + " ADD COLUMN thumbnail_height integer");
//						db.execSQL("ALTER TABLE " + TABLE_IN_MEMORY_MESSAGES + " ADD COLUMN thumbnail_width integer");
//						db.execSQL("ALTER TABLE " + TABLE_IN_MEMORY_MESSAGES + " ADD COLUMN thumbnail_height integer");
//
//						db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_STICKERS + " (" + "_id INTEGER PRIMARY KEY,"
//								+ "name TEXT," + "category TEXT," + "state INTEGER," + "show_order INTEGER,"
//								+ "UNIQUE (show_order)" + ");");
//
//						db.setTransactionSuccessful();
//					} catch (Throwable ex) {
...
//					db.beginTransaction();
//					try {
//						db.execSQL("ALTER TABLE " + TABLE_MESSAGES + " ADD COLUMN sample_image_path TEXT");
//						db.execSQL("ALTER TABLE " + TABLE_IN_MEMORY_MESSAGES + " ADD COLUMN sample_image_path TEXT");
//						db.setTransactionSuccessful();
//					} catch (Throwable ex) {
//						break;
//					} finally {
...
//					db.beginTransaction();
//					try {
//						db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_THEMES + " (" + "_id INTEGER PRIMARY KEY,"
//								+ "name TEXT," + "category TEXT," + "UNIQUE (category)" + ");");
//
//						db.execSQL("DROP TABLE IF EXISTS " + TABLE_STICKERS);
//						db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_STICKERS + " (" + "_id INTEGER PRIMARY KEY,"
//								+ "name TEXT," + "category TEXT," + "state INTEGER," + "show_order INTEGER,"
//								+ "scount INTEGER," // added
//								+ "UNIQUE (category)" + ");");
//						db.setTransactionSuccessful();
//					} catch (Throwable ex) {
//						break;
...
//					db.beginTransaction();
//					try {
//						db.execSQL("ALTER TABLE " + TABLE_MESSAGES + " ADD COLUMN plusimage_url TEXT");
//						db.execSQL("ALTER TABLE " + TABLE_MESSAGES + " ADD COLUMN plusimage_num INTEGER");
//						db.execSQL("ALTER TABLE " + TABLE_MESSAGES + " ADD COLUMN pluslink_url TEXT");
//
//						db.execSQL("ALTER TABLE " + TABLE_IN_MEMORY_MESSAGES + " ADD COLUMN plusimage_url TEXT");
//						db.execSQL("ALTER TABLE " + TABLE_IN_MEMORY_MESSAGES + " ADD COLUMN plusimage_num INTEGER");
//						db.execSQL("ALTER TABLE " + TABLE_IN_MEMORY_MESSAGES + " ADD COLUMN pluslink_url TEXT");
//						db.setTransactionSuccessful();
//					} catch (Throwable ex) {
//						break;
//					} finally {
...
//					db.beginTransaction();
//					try {
//						db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN phone TEXT");
//						db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN cover_image_count INTEGER");
//						db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN follow_count INTEGER");
//						db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN home_url TEXT");
//						db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN website TEXT");
//						db.execSQL("ALTER TABLE " + TABLE_CONTACTS + " ADD COLUMN hash TEXT");
//
//						db.execSQL("ALTER TABLE " + TABLE_PROFILE + " ADD COLUMN hash TEXT");
//						db.execSQL("ALTER TABLE " + TABLE_PROFILE + " ADD COLUMN facebookid TEXT");
//						db.execSQL("ALTER TABLE " + TABLE_PROFILE + " ADD COLUMN userid TEXT");
//						db.execSQL("ALTER TABLE " + TABLE_PROFILE + " ADD COLUMN email TEXT");
//						db.execSQL("ALTER TABLE " + TABLE_PROFILE + " ADD COLUMN email_password TEXT");
//						db.execSQL("ALTER TABLE " + TABLE_PROFILE + " ADD COLUMN email_logined TEXT");
//						db.execSQL("ALTER TABLE " + TABLE_PROFILE + " ADD COLUMN allow_add_me TEXT");
//
//						db.setTransactionSuccessful();
//					} catch (Throwable ex) {
//						break;
...
//			}
		...
		private void destroyOldTables(SQLiteDatabase db) {
			...
			db.execSQL("DROP TABLE IF EXISTS " + TABLE_AVATARS_BIG);
		...
		public void getAccountCount(SQLiteDatabase db) {
	...
	static {
		...
		sProviderAccountsProjectionMap.put(Imps.Provider.FULLNAME, "providers.fullname AS fullname");
		sProviderAccountsProjectionMap.put(Imps.Provider.CATEGORY, "providers.category AS category");
		sProviderAccountsProjectionMap.put(Imps.Provider.ACTIVE_ACCOUNT_ID, "accounts._id AS account_id");
		...
		sContactsProjectionMap.put(Imps.Contacts.SUBSCRIPTION_TYPE, "contacts.subscriptionType as subscriptionType");
		...
		sContactsProjectionMap.put(Imps.Contacts.ISDELIVERD, "contacts.is_delivered as is_delivered");
		sContactsProjectionMap.put(Imps.Contacts.DELIVERTYPE, "contacts.deliverType as deliverType");
		sContactsProjectionMap.put(Imps.Contacts.STATUS, "contacts.status as status");
		sContactsProjectionMap.put(Imps.Contacts.STATUSMESSAGE, "contacts.status as statusMessage");
		sContactsProjectionMap.put(Imps.Contacts.GENDER, "contacts.gender as gender");
		sContactsProjectionMap.put(Imps.Contacts.REGION, "contacts.region as region");
		sContactsProjectionMap.put(Imps.Contacts.FAVORITE, "contacts.favor as favor");
		sContactsProjectionMap.put(Imps.Contacts.ISLOADEDVCARD, "contacts.isloadedvcard as isloadedvcard");
		sContactsProjectionMap.put(Imps.Contacts.SUBSCRIPTIONMESSAGE,
				"contacts.subscriptionMessage as subscriptionMessage");
		sContactsProjectionMap.put(Imps.Contacts.PHONE_NUMBER, "contacts.phone as phone");
		sContactsProjectionMap.put(Imps.Contacts.COVER_IMAGE_COUNT, "contacts.cover_image_count as cover_image_count");
		sContactsProjectionMap.put(Imps.Contacts.FOLLOW_COUNT, "contacts.follow_count as follow_count");
		sContactsProjectionMap.put(Imps.Contacts.HOME_URL, "contacts.home_url as home_url");
		sContactsProjectionMap.put(Imps.Contacts.WEBSITE, "contacts.website as website");
		sContactsProjectionMap.put(Imps.Contacts.HASH, "contacts.hash as hash");
		sContactsProjectionMap.put(Imps.Contacts.FULLNAME, "contacts.fullname as fullname");
		sContactsProjectionMap.put(Imps.Contacts.DESCRIPTION, "contacts.description as description");
		sContactsProjectionMap.put(Imps.Contacts.MOBILE, "contacts.mobile as mobile");
		sContactsProjectionMap.put(Imps.Contacts.STAR, "contacts.star as star");
		sContactsProjectionMap.put(Imps.Contacts.USERID, "contacts.userid as userid");
		...
		sContactsProjectionMap.put(Imps.Contacts.PRESENCE_CUSTOM_STATUS, "presence.status AS status");
		sContactsProjectionMap.put(Imps.Contacts.CLIENT_TYPE, "presence.client_type AS client_type");
		...
		sContactsProjectionMap.put(Imps.Contacts.LAST_MESSAGE_DATE, "chats.last_message_date AS last_message_date");
		sContactsProjectionMap.put(Imps.Chats.NOTIFICATION, "chats.notification AS notification");
		sContactsProjectionMap.put(Imps.Chats.CHAT_ROOM_BACKGROUND, "chats.chat_room_background AS chat_room_background");
		...
		sContactsProjectionMap.put(Imps.Contacts.THUMBNAILPATH, "contacts.thumbnail_path as thumbnail_path");
		...
		sMessagesProjectionMap.put(Imps.Messages.SERVER_TIME, "messages.server_time AS server_time");
		...
		sMessagesProjectionMap.put(Imps.Messages.FILE_SUB_COUNT, "messages.file_sub_count AS file_sub_count");
		sMessagesProjectionMap.put(Imps.Messages.FILE_TOTAL_COUNT, "messages.file_total_count AS file_total_count");
		...
		sMessagesProjectionMap.put(Imps.Messages.TIME_DELAY, "messages.time_delay AS time_delay");
		sMessagesProjectionMap.put(Imps.Messages.THUMBNAIL_WIDTH, "messages.thumbnail_width AS thumbnail_width");
		sMessagesProjectionMap.put(Imps.Messages.THUMBNAIL_HEIGHT, "messages.thumbnail_height AS thumbnail_height");
		sMessagesProjectionMap.put(Imps.Messages.MIME_TYPE, "messages.mime_type AS mime_type");
		sMessagesProjectionMap.put(Imps.Messages.SAMPLE_IMAGE_PATH, "messages.sample_image_path AS sample_image_path");
		sMessagesProjectionMap.put(Imps.Messages.PLUSIMAGE_URL, "messages.plusimage_url AS plusimage_url");
		sMessagesProjectionMap.put(Imps.Messages.PLUSIMAGE_NUM, "messages.plusimage_num AS plusimage_num");
		sMessagesProjectionMap.put(Imps.Messages.PLUSLINK_URL, "messages.pluslink_url AS pluslink_url");
		...
		sInMemoryMessagesProjectionMap.put(Imps.Messages.THREAD_ID, "inMemoryMessages.thread_id AS thread_id");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.PACKET_ID, "inMemoryMessages.packet_id AS packet_id");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.NICKNAME, "inMemoryMessages.nickname AS nickname");
		...
		sInMemoryMessagesProjectionMap.put(Imps.Messages.SERVER_TIME, "inMemoryMessages.server_time AS server_time");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.TYPE, "inMemoryMessages.type AS type");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.ERROR_CODE, "inMemoryMessages.err_code AS err_code");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.ERROR_MESSAGE, "inMemoryMessages.err_msg AS err_msg");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.FILE_SUB_COUNT, "inMemoryMessages.file_sub_count AS file_sub_count");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.FILE_TOTAL_COUNT, "inMemoryMessages.file_total_count AS file_total_count");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.IS_GROUP_CHAT, "inMemoryMessages.is_muc AS is_muc");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.DISPLAY_SENT_TIME, "inMemoryMessages.show_ts AS show_ts");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.IS_DELIVERED, "inMemoryMessages.is_delivered AS is_delivered");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.TIME_DELAY, "inMemoryMessages.time_delay AS time_delay");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.THUMBNAIL_WIDTH,
				"inMemoryMessages.thumbnail_width AS thumbnail_width");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.THUMBNAIL_HEIGHT,
				"inMemoryMessages.thumbnail_height AS thumbnail_height");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.MIME_TYPE, "inMemoryMessages.mime_type AS mime_type");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.SAMPLE_IMAGE_PATH,
				"inMemoryMessages.sample_image_path AS sample_image_path");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.PLUSIMAGE_URL,
				"inMemoryMessages.plusimage_url AS plusimage_url");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.PLUSIMAGE_NUM,
				"inMemoryMessages.plusimage_num AS plusimage_num");
		sInMemoryMessagesProjectionMap.put(Imps.Messages.PLUSLINK_URL, "inMemoryMessages.pluslink_url AS pluslink_url");
	...
	private void setupImUrlMatchers(String authority) {
		...
		mUrlMatcher.addURI(authority, "messagesById/#", MATCH_MESSAGES_BY_ID);
		...
		mUrlMatcher.addURI(authority, "otrMessagesByAcctAndContact/#/*", MATCH_OTR_MESSAGES_BY_CONTACT);
		...
		mUrlMatcher.addURI(authority, "big_avatars", MATCH_AVATARS_BIG);
		mUrlMatcher.addURI(authority, "big_avatars/#", MATCH_AVATAR_BIG);
		mUrlMatcher.addURI(authority, "big_avatarsBy/#/#", MATCH_AVATAR_BIG_BY_PROVIDER);
		...
		mUrlMatcher.addURI(authority, "providerSettings/#/*", MATCH_PROVIDER_SETTINGS_BY_ID_AND_NAME);
		...
		mUrlMatcher.addURI(authority, "CONTACT", MATCH_UNICONTACTS);
		mUrlMatcher.addURI(authority, "unicontacts", MATCH_VIEW_UNICONTACTS);
		mUrlMatcher.addURI(authority, "notifications", MATCH_NOTIFICATIONS);
		mUrlMatcher.addURI(authority, "profile", MATCH_PROFILE);
		// mUrlMatcher.addURI(authority, "groupProfile", MATCH_GROUPPROFILE);
		mUrlMatcher.addURI(authority, "stickers", MATCH_STICKERS);
		mUrlMatcher.addURI(authority, "themes", MATCH_THEMES);
		mUrlMatcher.addURI(authority, "tags", MATCH_TAGS);
	}
...
//		SQLiteDatabase.loadLibs(getContext());
	...
	private void setDatabaseName() {
		mDatabaseName = UNENCRYPTED_DATABASE_NAME;
		mTransientDbName = "transient_" + mDatabaseName.replace(".", "_");
	}

	private synchronized DatabaseHelper initDBHelper(boolean noCreate) throws Exception {
		if (mDbHelper == null) {
			setDatabaseName();
			...
			mDbHelper = new DatabaseHelper(ctx, mDatabaseName, null, mDatabaseVersion);
		}
	...
	public Cursor queryInternal(Uri url, String[] projectionIn, String selection, String[] selectionArgs, String sort) {
		...
		limit = url.getQueryParameter("LIMIT");
		...
		case MATCH_MESSAGES_BY_ID:
			...
			qb.setTables(TABLE_MESSAGES);
			{
				...
				String tmpQuery = qb.buildUnionQuery(new String[]{query1, query2}, sort, limit);
				if (limit != null && offset != null) {
					tmpQuery = String.format("%s OFFSET %s", tmpQuery, offset);
				}

				final String query = tmpQuery;

				final SQLiteDatabase readdb = getDBHelper().getReadableDatabase();
				String[] doubleArgs = null;
				if (selectionArgs != null) {

					doubleArgs = new String[selectionArgs.length * 2];// Arrays.copyOf(selectionArgs,
				...
				Cursor c = readdb.rawQueryWithFactory(null, query, doubleArgs, TABLE_MESSAGES);
		...
		case MATCH_MESSAGES_BY_CONTACT:
			...
			final SQLiteDatabase db2 = getDBHelper().getReadableDatabase();
		...
		case MATCH_OTR_MESSAGES_BY_PACKET_ID:
			/*
			 * try{ String packetId =
			 * decodeURLSegment(url.getPathSegments().get(1));
			 * qb.setTables(TABLE_MESSAGES);
			 * qb.setProjectionMap(sMessagesProjectionMap);
			...
			qb.setTables(TABLE_MESSAGES);
			qb.setProjectionMap(sMessagesProjectionMap);
			appendWhere(whereClause, Imps.Messages.PACKET_ID, "='" + packetId + "'", null);

			final String sell = whereClause.toString();
			@SuppressWarnings("deprecation")
			final String q11 = qb.buildQuery(projectionIn, sell, null, null, null, null, null);
			...
			qb.setProjectionMap(sInMemoryMessagesProjectionMap);
			@SuppressWarnings("deprecation")
			final String q22 = qb.buildQuery(projectionIn, sell, null, null, null, null, null);

			// Put them together
			final String q33 = qb.buildUnionQuery(new String[] { q11, q22 }, sort, null);
			final SQLiteDatabase db22 = getDBHelper().getReadableDatabase();
			Cursor c3 = db22.rawQueryWithFactory(null, q33, null, TABLE_MESSAGES);
		...
		case MATCH_AVATAR_BY_PROVIDER:
			...
			break;
		case MATCH_AVATARS_BIG:
			qb.setTables(TABLE_AVATARS_BIG);
			break;

		case MATCH_AVATAR_BIG_BY_PROVIDER:
			qb.setTables(TABLE_AVATARS_BIG);
			appendWhere(whereClause, Imps.Big_Avatars.ACCOUNT, "=", url.getPathSegments().get(2));
			break;
		case MATCH_CHATS:
		...
		case MATCH_CHATS_BY_ACCOUNT:
			...
			appendWhere(whereClause,
					buildContactIdSelection(Imps.Chats.CONTACT_ID, Imps.Contacts.ACCOUNT + "='" + accountStr + "'"));
			break;
		...
		case MATCH_UNICONTACTS:
			qb.setTables(TABLE_UNICONTACTS);
			break;
		case MATCH_VIEW_UNICONTACTS:
			qb.setTables(VIEW_UNICONTACTS);
			break;
		...
		case MATCH_PROFILE:
			...
			break;
		/*
		 * case MATCH_GROUPPROFILE: qb.setTables(TABLE_GROUPPROFILE); break;
		 */ case MATCH_STICKERS:
			qb.setTables(TABLE_STICKERS);
			break;
		case MATCH_THEMES:
			qb.setTables(TABLE_THEMES);
			break;
		case MATCH_TAGS:
		...
		try {
			...
			// Failed to actually open - the passphrase must have been wrong -
			// reset the helper
			mDbHelper = null;
			throw e;
		}
	...
	static class MyCrossProcessCursorWrapper
		...
		/*
		 * @Override public CursorWindow getWindow() { return null; }
		 * 
		 * @Override public void fillWindow(int position, CursorWindow window) {
		 * if (position < 0 || position > getCount()) { return; }
		 * window.acquireReference(); try { moveToPosition(position - 1);
		 * window.clear(); window.setStartPosition(position); int columnNum =
		 * getColumnCount(); window.setNumColumns(columnNum); while
		 * (moveToNext() && window.allocRow()) { for (int i = 0; i < columnNum;
		 * i++) { String field = getString(i); if (field != null) { if
		 * (!window.putString(field, getPosition(), i)) { window.freeLastRow();
		 * break; } } else { if (!window.putNull(getPosition(), i)) {
		 * window.freeLastRow(); break; } } } } } catch (IllegalStateException
	...
	private void buildQueryContactsByProvider(SQLiteQueryBuilder qb, StringBuilder whereClause, Uri url) {
		qb.setTables(CONTACT_JOIN_PRESENCE_CHAT_AVATAR_TABLE);
		qb.setProjectionMap(sContactsProjectionMap);
	...
	public String getType(Uri url) {
		...
		case MATCH_AVATARS_BIG:
			return Imps.Big_Avatars.CONTENT_TYPE;

		case MATCH_AVATAR_BIG:
			return Imps.Big_Avatars.CONTENT_ITEM_TYPE;
		case MATCH_CHATS:
	...
	boolean insertBulkContacts(SQLiteDatabase db, ContentValues values) {
		...
		if (usernameCount != nicknameCount) {
			LogCleaner.warn(LOG_TAG,
					"[ImProvider] insertBulkContacts: input bundle " + "username & nickname lists have diff. length!");
			return false;
		}

		ArrayList<String> favoriteArray = getStringArrayList(values, Imps.Contacts.FAVORITE);
		...
		ArrayList<String> useridArray = getStringArrayList(values, Imps.Contacts.USERID);
		int sum = 0;
		...
		for (int i = 0; i < usernameCount; i++) {
			...
			String userId = useridArray.get(i);
			int type = 0;
			...
			try {
				...
				if (subscriptionStatusArray != null) {
					String strStatus = subscriptionStatusArray.get(i);
					if (strStatus == null)
						subscriptionStatus = Imps.Contacts.SUBSCRIPTION_STATUS_NONE;
					else
						subscriptionStatus = Integer.parseInt(strStatus);
				}
				if (subscriptionTypeArray != null) {
					String strType = subscriptionTypeArray.get(i);

					if (strType == null)
						subscriptionType = Imps.Contacts.SUBSCRIPTION_TYPE_BOTH;
					else
						subscriptionType = Integer.parseInt(strType);
				}
			...
			try {
				favor = Integer.parseInt(favoriteArray.get(i));
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			contactValues.put(Imps.Contacts.FULLNAME, nickname);
			contactValues.put(Imps.Contacts.TYPE, type);
			contactValues.put(Imps.Contacts.FAVORITE, favor);
			contactValues.put(Imps.Contacts.USERID, userId);
		...
		// We know that we succeeded becuase endTransaction throws if the
		// transaction failed.
		if (DBG)
	...
	int updateBulkContacts(SQLiteDatabase db, ContentValues values, String userWhere) {
		ArrayList<String> usernames = getStringArrayList(values, Imps.Contacts.USERNAME);
		ArrayList<String> nicknames = getStringArrayList(values, Imps.Contacts.NICKNAME);
		...
		if (usernameCount != nicknameCount) {
			LogCleaner.warn(LOG_TAG,
					"[ImProvider] updateBulkContacts: input bundle " + "username & nickname lists have diff. length!");
			return 0;
		}
		...
		for (int i = 0; i < usernameCount; i++) {
			...
			if (DBG)
				log("updateBulkContacts[" + i + "] username=" + username + ", nickname=" + nickname + ", type=" + type
			...
			if (numUpdated == 0) {
				LogCleaner.warn(LOG_TAG,
						"[ImProvider] updateBulkContacts: " + " update failed for selection = " + updateSelection);
			} else {
				sum += numUpdated;
			}

			// yield the cii_lock if anyone else is trying to
	 ...
	 * this method does not cii_remove presences for which the corresponding
	 ...
	 * 
	 * @param account
	 *            the account of the contacts for which we want to create seed
	 *            presence rows.
	 */
	// @TargetApi(Build.VERSION_CODES.HONEYCOMB)
	private void seedInitialPresenceByAccount(SQLiteDatabase db, long account) {
		SQLiteQueryBuilder qb = new SQLiteQueryBuilder();
		qb.setTables(TABLE_CONTACTS);
		qb.setProjectionMap(sContactsProjectionMap);
		...
		// for in-memory presence table, add a presence row for each contact
		// that
		// doesn't have a presence. in-memory presence table isn't reliable, and
		// goes away
		// when device reboot or IMProvider process dies, so we can't rely on
		// each contact
		...
		c = qb.query(getDBHelper().getReadableDatabase(), CONTACT_ID_PROJECTION, CONTACTS_WITH_NO_PRESENCE_SELECTION,
		...
		while (c.moveToNext()) {
			...
			try {
				...
				// we could possibly catch this exception, since there could be
				// a presence
				// row with the same contact_id. That's fine, just ignore the
				// error
				if (DBG)
	...
	private int updateBulkPresence(SQLiteDatabase db, ContentValues values, String userWhere, String[] whereArgs) {
		ArrayList<String> usernames = getStringArrayList(values, Imps.Contacts.USERNAME);
		int count = usernames.size();
		Long account = values.getAsLong(Imps.Contacts.ACCOUNT);

		// ArrayList<String> priorityArray = getStringArrayList(values,
		// Imps.Presence.PRIORITY);
		ArrayList<String> modeArray = getStringArrayList(values, Imps.Presence.PRESENCE_STATUS);
		// ArrayList<String> statusArray =
		// getStringArrayList(values,Imps.Presence.PRESENCE_CUSTOM_STATUS);
		...
		for (int i = 0; i < count; i++) {
			...
			try {
				/*
				 * if (priorityArray != null) { priority =
				 * Integer.parseInt(priorityArray.get(i)); }
				 */
				if (modeArray != null) {
			...
			if (DBG) {
				log("updateBulkPresence[" + i + "] username=" + username + ", mode=" + mode + ", resource="
			...
			int numUpdated = db.update(TABLE_PRESENCE, presenceValues, selection, selectionArgs);
			if (numUpdated == 0) {
				// this is really generating a lot of log output that doesn't
				// seem necessary
				// LogCleaner.warn(LOG_TAG, "[ImProvider] updateBulkPresence:
				// failed for " + username);
			} else {
				sum += numUpdated;
			}
	...
	private Uri insertInternal(Uri url, ContentValues initialValues) {
...

		{
			if (db.isOpen()) {
				...
				try {
					...
					case MATCH_CONTACTS_BULK:
						if (insertBulkContacts(db, initialValues)) {
							// notify change using the "content://im/contacts"
							// url,
							// so the change will be observed by listeners
							// interested
							// in contacts changes.
					...
					case MATCH_MESSAGES_BY_CONTACT:
						...
						initialValues.put(Imps.Messages.THREAD_ID,
								getContactId(getDBHelper().getReadableDatabase(), accountStr, contact));
					...
					case MATCH_OTR_MESSAGES_BY_CONTACT:
						...
						contact = decodeURLSegment(url.getPathSegments().get(2));
						initialValues.put(Imps.Messages.THREAD_ID,
								getContactId(getDBHelper().getReadableDatabase(), accountStr2, contact));
					...
					case MATCH_AVATARS:
						...
						break;
					case MATCH_AVATAR_BIG_BY_PROVIDER:
						appendValuesFromUrl(initialValues, url, Imps.Big_Avatars.PROVIDER, Imps.Big_Avatars.ACCOUNT);
					case MATCH_AVATARS_BIG:
						rowID = db.replace(TABLE_AVATARS_BIG, "contact", initialValues);
						if (rowID > 0) {
							resultUri = Uri.parse(Imps.Big_Avatars.CONTENT_URI + "/" + rowID);
						}
						break;
					case MATCH_CHATS_ID:
					...
					case MATCH_CHATS:
						...
						notifyContactContentUri = true;
						notifyProviderAccountContentUri = true; // For updating
																// account stats
																// in account
																// list
						break;
					...
					case MATCH_PRESENCE_SEED_BY_ACCOUNT:
						try {
							seedInitialPresenceByAccount(db, Long.parseLong(url.getLastPathSegment()));
					...
					case MATCH_UNICONTACTS:
						rowID = db.insert(TABLE_UNICONTACTS, null, initialValues);
						if (rowID > 0) {
							resultUri = Uri.parse(Imps.UniContacts.CONTENT_URI + "/" + rowID);
						}
						break;
					case MATCH_NOTIFICATIONS:
						rowID = db.insert(TABLE_NOTIFICATIONS, null, initialValues);
						if (rowID > 0) {
							resultUri = Uri.parse(Imps.Notifications.CONTENT_URI + "/" + rowID);
						}
						break;
					case MATCH_PROFILE:
						rowID = db.insert(TABLE_PROFILE, null, initialValues);
						if (rowID > 0) {
							resultUri = Uri.parse(Imps.Profile.CONTENT_URI + "/" + rowID);
						}
						break;
					/*
					 * case MATCH_GROUPPROFILE: rowID =
					 * db.insert(TABLE_GROUPPROFILE, null, initialValues); if
					 * (rowID > 0) { resultUri =
					 * Uri.parse(Imps.GroupProfile.CONTENT_URI + "/" + rowID); }
					 * break;
					 */
					case MATCH_STICKERS:
						rowID = db.insert(TABLE_STICKERS, null, initialValues);
						if (rowID > 0) {
							resultUri = Uri.parse(Imps.Stickers.CONTENT_URI + "/" + rowID);
						}
						break;

					case MATCH_THEMES:
						rowID = db.insert(TABLE_THEMES, null, initialValues);
						if (rowID > 0) {
							resultUri = Uri.parse(Imps.Themes.CONTENT_URI + "/" + rowID);
						}
						break;

					case MATCH_TAGS:
						rowID = db.insert(TABLE_TAGS, null, initialValues);
						if (rowID > 0) {
							resultUri = Uri.parse(Imps.Tags.CONTENT_URI + "/" + rowID);
						}
						break;
		...
		if (resultUri != null) {
			...
			// In most case, we query contacts with presence and chats joined,
			// thus
			// we should also notify that contacts changes when presence or
			// chats changed.
	...
	private long getContactId(final SQLiteDatabase db, final String accountId, final String contact) {
		...
		Cursor c = qb.query(db, CONTACT_ID_PROJECTION, CONTACT_ID_QUERY_SELECTION, mQueryContactIdSelectionArgs2, null,
				null, null, null);
	...
	// The chat UI provides slots (0, 9, .., 1) for the first 10 chats. This
	// allows you to
	// quickly switch between these chats by chording menu+#. We number from the
	// right end of
	// the number row and move leftward to make an easier two-hand chord with
	// the menu button
	// on the left side of the keyboard.
	private void addToQuickSwitch(long newRow) {
		// Since there are fewer than 10, there must be an empty slot. Let's
		// find it.
	...
	// If there are more than 10 chats and one with a quick switch slot ends
	// then pick a chat
	...
	private void backfillQuickSwitchSlots() {
		...
		Cursor c = query(Imps.Chats.CONTENT_URI, BACKFILL_PROJECTION, Imps.Chats.SHORTCUT + "=-1", null,
				Imps.Chats.LAST_MESSAGE_DATE + " DESC");
	...
	private int updateSlotForChat(long chatId, int slot) {
		...
		return update(Imps.Chats.CONTENT_URI, values, Imps.Chats._ID + "=?", new String[] { Long.toString(chatId) });
	}
	...
	private int findEmptyQuickSwitchSlot() {
		...
		try {
			// If there are 10 or more chats then all the quick switch slots are
			// already filled
			if (N >= 10) {
				return -1;
			}
			...
			// The map is here because numbers go from 0-9, but we want to
			// assign slots in
			// 0, 9, 8, ..., 1 order to match the right-to-left reading of the
			// number row
			// on the keyboard.
			int[] map = new int[] { 0, 9, 8, 7, 6, 5, 4, 3, 2, 1 };

			// Mark all the slots that are in use
			// The shortcuts represent actual keyboard number row keys, and not
			// ordinals.
			// So 7 would mean the shortcut is the 7 key on the keyboard and NOT
			// the 7th
			// shortcut. The passing of slot through map[] below maps these
			// keyboard key
			...
			// As we exit this, the push of i through map[] maps the ordinal bit
			// position
			// in the 'slots' bitfield onto a key on the number row of the
			// device keyboard.
	...
	private static final String DELETE_PRESENCE_SELECTION = Imps.Presence.CONTACT_ID + " in (select "
			+ PRESENCE_CONTACT_ID + " from " + TABLE_PRESENCE + " left outer join " + TABLE_CONTACTS + " on "
			+ PRESENCE_CONTACT_ID + '=' + CONTACT_ID + " where " + CONTACT_ID + " IS NULL)";

	// private static final String CHATS_CONTACT_ID = TABLE_CHATS + '.' +
	// Imps.Chats.CONTACT_ID;
	/*
	 * private static final String DELETE_CHATS_SELECTION =
	 * Imps.Chats.CONTACT_ID + " in (select " + CHATS_CONTACT_ID + " from " +
	 * TABLE_CHATS + " left outer join " + TABLE_CONTACTS + " on " +
	 * CHATS_CONTACT_ID + '=' + CONTACT_ID + " where " + CONTACT_ID +
	 * " IS NULL)";
	 */

	private static final String GROUP_MEMBER_ID = TABLE_GROUP_MEMBERS + '.' + Imps.GroupMembers.GROUP;
	private static final String DELETE_GROUP_MEMBER_SELECTION = Imps.GroupMembers.GROUP + " in (select "
			+ GROUP_MEMBER_ID + " from " + TABLE_GROUP_MEMBERS + " left outer join " + TABLE_CONTACTS + " on "
			+ GROUP_MEMBER_ID + '=' + CONTACT_ID + " where " + CONTACT_ID + " IS NULL)";

	// private static final String GROUP_MESSAGES_ID = TABLE_MESSAGES + '.' +
	 ...
	 * GROUP_MESSAGES_ID + '=' + CONTACT_ID + " where " + CONTACT_ID +
	 * " IS NULL)";
	 */

	private void performContactRemovalCleanup(SQLiteDatabase db, long contactId) {
		if (contactId > 0) {
			StringBuilder buf = new StringBuilder();
			DebugConfig.info("performContactRemovalCleanup", "contactId:" + String.valueOf(contactId));
			// delete presence
	...
	private void deleteWithSelection(SQLiteDatabase db, String tableName, String selection, String[] selectionArgs) {
		if (DBG)
	...
	private int deleteInternal(Uri url, String userWhere, String[] whereArgs) {
		...
		case MATCH_MESSAGES_BY_ID:
			...
			appendWhere(whereClause, Imps.Messages._ID, "=", threadId);
		...
		case MATCH_MESSAGES_BY_PROVIDER:
			...
			appendWhere(whereClause,
					buildContactIdSelection(Imps.Messages.THREAD_ID, Imps.Contacts.PROVIDER + "='" + provider + "'"));
		...
		case MATCH_MESSAGES_BY_ACCOUNT:
			...
			accountStr = decodeURLSegment(url.getPathSegments().get(1));
			appendWhere(whereClause,
					buildContactIdSelection(Imps.Messages.THREAD_ID, Imps.Contacts.ACCOUNT + "='" + accountStr + "'"));
		...
		case MATCH_AVATAR_BY_PROVIDER:
			...
			break;
		case MATCH_AVATARS_BIG:
			tableToChange = TABLE_AVATARS_BIG;
			break;
		...
		case MATCH_AVATAR_BIG_BY_PROVIDER:
			tableToChange = TABLE_AVATARS_BIG;
			changedItemId = url.getPathSegments().get(2);
			idColumnName = Imps.Big_Avatars.ACCOUNT;
			break;
		case MATCH_CHATS:
			tableToChange = TABLE_CHATS;
			backfillQuickSwitchSlots = true;
			notifyProviderAccountContentUri = true; // For updating account
													// stats in account list
			break;
		...
		case MATCH_PRESENCE_BY_ACCOUNT:
			...
			accountStr = decodeURLSegment(url.getLastPathSegment());
			appendWhere(whereClause,
					buildContactIdSelection(Imps.Presence.CONTACT_ID, Imps.Contacts.ACCOUNT + "='" + accountStr + "'"));
		...
		case MATCH_UNICONTACTS:
			tableToChange = TABLE_UNICONTACTS;
			break;
		case MATCH_PROFILE:
			tableToChange = TABLE_PROFILE;
			break;
		/*
		 * case MATCH_GROUPPROFILE: tableToChange = TABLE_GROUPPROFILE; break;
		 */
		case MATCH_NOTIFICATIONS:
			tableToChange = TABLE_NOTIFICATIONS;
			break;

		case MATCH_STICKERS:
			tableToChange = TABLE_STICKERS;
			break;

		case MATCH_THEMES:
			tableToChange = TABLE_THEMES;
			break;

		case MATCH_OTR_MESSAGES_BY_PACKET_ID:
			tableToChange = TABLE_MESSAGES;
			tableToChange2 = TABLE_IN_MEMORY_MESSAGES;
			String packetId = url.getPathSegments().get(1);
			appendWhere(whereClause, Imps.Messages.PACKET_ID + "='" + packetId + "'");
			break;

		case MATCH_TAGS:
			tableToChange = TABLE_TAGS;
			break;
		...
		// synchronized (db)
		{
			if (db.isOpen()) {
				...
				try {
					count = db.delete(tableToChange, whereClause.toString(), whereArgs);
					// see the comment at the declaration of tableToChange2 for
					// an explanation
					if (tableToChange2 != null) {
						count += db.delete(tableToChange2, whereClause.toString(), whereArgs);
					}
					if (contactDeleted && count > 0) {
						// since the contact cleanup triggers no longer work for
						// cross database tables,
						// we have to do it by hand here.
						performContactRemovalCleanup(db, deletedContactId);
					}
					db.setTransactionSuccessful();
				} catch (Exception e) {
					e.printStackTrace();
		...
		if (count > 0) {
			...
			// we should also notify that contacts changes when presence or
			// chats changed.
			if (match == MATCH_CHATS || match == MATCH_CHATS_ID || match == MATCH_PRESENCE || match == MATCH_PRESENCE_ID
		...
		// synchronized (db)
		{
			if (db.isOpen()) {
				...
				try {
					...
					case MATCH_CONTACTS_BULK:
						count = updateBulkContacts(db, values, userWhere);
						if (count > 0) {
					...
					case MATCH_MESSAGES_BY_CONTACT:
						...
						try {
							contactId = getContactId(getDBHelper().getReadableDatabase(), accountStr, contact);
						} catch (Exception e) {
							e.printStackTrace();
						}

						if (whereClause != null && contactId > 0) {
							appendWhere(whereClause, Imps.Messages.THREAD_ID, "=", contactId);
							notifyMessagesContentUri = true;
						}
					...
					case MATCH_GROUP_MEMBERS_BY_GROUP:
						tableToChange = TABLE_GROUP_MEMBERS;
						changedItemId = url.getPathSegments().get(1);
						idColumnName = Imps.GroupMembers.GROUP;
						break;
					...
					case MATCH_PRESENCE_BULK:
						try {
							count = updateBulkPresence(db, values, userWhere, whereArgs);
							if (count > 0) {
								getContext().getContentResolver().notifyChange(Imps.Contacts.CONTENT_URI, null);
								getContext().getContentResolver().notifyChange(Imps.GroupMembers.CONTENT_URI, null);
							}
						} catch (Exception e) {
							e.printStackTrace();
						}
					...
					case MATCH_PROFILE:
						...
						break;
					/*
					 * case MATCH_GROUPPROFILE: tableToChange =
					 * TABLE_GROUPPROFILE; break;
					 */
					case MATCH_STICKERS:
						tableToChange = TABLE_STICKERS;
						break;
				...
				try {
					if (match == MATCH_PRESENCE_BULK || match == MATCH_CONTACTS_BULK) {
						log("update : bulk update");
					} else {
	...
	private static void appendWhere(StringBuilder where, String columnName, String condition, Object value) {
		if (where.length() > 0) {
			where.append(" AND ");
		}
	...
	private void setEncryptedPassword() {
		...
		password = Utilities.MD5(Utils.getIMEI());
		if (password == null) password = "RSVcSproVidERDEFKeY123";
