...
public abstract class ImConnection {
    ...
    {
        doNotifyContactsPresenceUpdated(contact);
    }
     ...
     * client. Only sessions that were dropped unexpectedly(e.g. power loss,
     * crash, etc) can be re-established using the stored session context. If
     * the session was terminated normally by either user logging out or server
    ...
    protected abstract void doNotifyContactsPresenceUpdated(Contact contact);
    ...
    public abstract FindFriendManager getFindFriendManager();
    public abstract void sendLocation(double latitude, double longitude);
    //public abstract void loadVcards();
    public abstract void loadVcard(String address, String hash, boolean needName);
    public abstract boolean isPicaConnection();
}
