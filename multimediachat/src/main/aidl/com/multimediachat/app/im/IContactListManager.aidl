...
interface IContactListManager {
     ...
     * date:2014/4/29
     */
	void requestSubscription(in Contact contact, in String greeting);
    ...
    void cancelRequest(in Contact address);
	...
	/**
     * Hides a contact. The ContactListListener will be notified when the contact is hidden
     * successfully or any error occurs.
     * @author: cys
     * @param address the address of the contact to block.
     * @return ILLEGAL_CONTACT_LIST_MANAGER_STATE if contact lists is not loaded.
	...
	/**
     * Unhides a contact.The ContactListListener will be notified when the contact is unhidden
     * successfully or any error occurs.
     *
     * @param address the address of the contact to unblock.
     * @return ILLEGAL_CONTACT_LIST_MANAGER_STATE if contact lists is not loaded.
     */
    int unHideContact(String address);
    
    int favoriteContact(String address);
    int unFavoriteContact(String address);
    ...
    int getSubRequestedFriendCount();
    
    /**
     * 친구들을 더 불러와 자료기지에 보관한다.
     */
    boolean loadMoreFriends();
    
    boolean addContact(in Contact contact);
    boolean addContactWithDB(in Contact contact);
}
